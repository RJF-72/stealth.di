...
export function CodeFontLoader() {
  const { diModel, loadedCodeFonts } = useStealthDI();
  const [loading, setLoading] = useState<string[]>([]);

  const loadCodeFont = async (codefontId: string) => {
  ...
  const isLoaded = (codefontId: string) => 
    loadedCodeFonts.some(cf => cf.id === codefontId);

  const isLoading = (codefontId: string) => 
    loading.includes(codefontId);

  return (
    <div className="glass-panel medium">
      <h3 className="text-stealth-primary mb-4">🧠 Available CodeFonts</h3>
      
      <div className="space-y-3">
        {AVAILABLE_CODEFONTS.map((codefont) => (
          <div
            key={codefont.id}
            className={`p-4 rounded-lg border transition-all ${
              isLoaded(codefont.id)
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  {codefont.capabilities.map((capability) => (
                    <span
                      key={capability}
                      className="px-2 py-1 text-xs bg-stealth-surface rounded"
                    >
                      {capability}
                    </span>
                  ))}
              ...
              <div className="text-right">
                ...
                <button
                  onClick={() => loadCodeFont(codefont.id)}
                  disabled={isLoaded(codefont.id) || isLoading(codefont.id)}
                  className={`stealth-btn ${
                    isLoaded(codefont.id) ? 'success' : 'primary'
                  }`}
                >
      ...
      <div className="mt-4 p-3 bg-stealth-surface rounded">
        ...
        <div className="flex gap-2 flex-wrap">
          {loadedCodeFonts.map((cf) => (
            <span
              key={cf.id}
              className="px-3 py-1 bg-stealth-primary/20 text-stealth-primary rounded-full text-sm"
            >
              {cf.name} ✅
            </span>
          ))}
          {loadedCodeFonts.length === 0 && (
            <span className="text-stealth-muted text-sm">
